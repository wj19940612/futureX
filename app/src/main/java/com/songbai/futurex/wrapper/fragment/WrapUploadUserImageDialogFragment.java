...
public class WrapUploadUserImageDialogFragment extends BottomDialogFragment {
        ...
        switch (view.getId()) {
            ...
            case R.id.lookHDPicture:
                Launcher.with(getActivity(), LookBigPictureActivity.class)
                        .putExtra(ExtraKeys.EX_PAYLOAD, HDPictureUrl)
                        .execute();
                this.dismissAllowingStateLoss();
                break;
        }
    ...
    private void openGalleryPage() {
        if (mImageDealType == IMAGE_TYPE_OPEN_CUSTOM_GALLERY) {
            ...
            openGalleryIntent.putExtra(ExtraKeys.EX_PAYLOAD, 0);
...
//        }
        ...
        if (resultCode == FragmentActivity.RESULT_OK) {
            switch (requestCode) {
                ...
                case REQ_CODE_AREA_TAKE_PHONE:
                    String imageUrl = data.getStringExtra(ExtraKeys.EX_PAYLOAD);
                    if (!TextUtils.isEmpty(imageUrl)) {
                        dealImagePath(imageUrl);
                    }
                    break;
                case REQ_CODE_TAKE_PHONE_FROM_GALLERY:
                    String imagePath = data.getStringExtra(ExtraKeys.EX_PAYLOAD);
    ...
    private void dealImagePath(String imaUri) {
        if (mImageDealType == IMAGE_TYPE_CLIPPING_IMAGE_SCALE_OR_MOVE) {
            ...
            intent.putExtra(WrapClipHeadImageActivity.KEY_CLIP_USER_IMAGE, imaUri);
