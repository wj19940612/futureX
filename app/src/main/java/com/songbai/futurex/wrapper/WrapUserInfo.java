 ...
 * 登录后的用户信息数据
 */
public class WrapUserInfo implements Parcelable {
    //	待审核0、审核通过1、审核未通过 2
    ...
    public void updateLocalUserInfo(UserDetailInfo userDetailInfo) {
        ...
        LocalWrapUser.getUser().setUserInfo(this);
    }
    ...
    protected WrapUserInfo(Parcel in) {
        this.id = in.readInt();
    ...
    public static final Creator<WrapUserInfo> CREATOR = new Creator<WrapUserInfo>() {
        @Override
        public WrapUserInfo createFromParcel(Parcel source) {
            return new WrapUserInfo(source);
        }

        @Override
        public WrapUserInfo[] newArray(int size) {
            return new WrapUserInfo[size];
        }
    };
